...
**Both a night mode and a day mode (مود ليلي + مود نهاري)** — **light is the signature default** (owner decision, 2026-09-04: the site must match the deck, which is light throughout), dark is a fully-polished equal, not an afterthought. *(Superseded the original dark-default call; the shipped palette lives in `resources/css/app.css` `:root`.)* Toggle persists per user (localStorage + user pref column) and honors `prefers-color-scheme`; both must pass WCAG contrast. Drive both from CSS custom properties in the `@theme` block so a single token swap flips the theme (no duplicated components).
