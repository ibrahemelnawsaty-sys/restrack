...
@section('title', 'لوحة السفير — Restrack')
...
@section('content')
  <section class="page">
    <nav class="glass tile rise in" style="display:flex;gap:6px;flex-wrap:wrap;padding:10px;margin-bottom:20px;align-items:center">
      <span class="btn btn-gold-line btn-sm">لوحة السفير</span>
      <a href="{{ route('home') }}" style="margin-inline-start:auto;padding:.5rem .8rem;text-decoration:none;color:var(--ink-3);font-weight:700;font-size:.82rem">← الموقع</a>
    </nav>

    <div class="page-head rise in">
      <span class="eyebrow"><svg class="ico" aria-hidden="true"><use href="#i-users"/></svg>سفير ريستراك</span>
      <h1>مرحباً، {{ $user->name }}</h1>
      <p>شارك رابط دعوتك مع طلابك — كل من يسجّل عبره يُنسَب إليك، وتتابع أعدادهم من هنا.</p>
    </div>

    <div class="glass tile rise in">
      <h3 style="margin-bottom:6px">رابط الدعوة الخاص بك</h3>
      <p style="color:var(--ink-2);font-size:.9rem;margin-bottom:12px">انسخه وأرسله لطلابك عبر واتساب أو غيره.</p>
      ...
      <div class="stat-row" style="margin-top:14px">
        <div class="glass stat"><div class="v num">{{ $referral['registered'] }}</div><div class="k">طالب سجّل عبرك</div></div>
        <div class="glass stat"><div class="v num">{{ $referral['subscribers'] }}</div><div class="k">مشترك فعّال</div></div>
      </div>
    </div>

    <div class="glass tile rise in">
      <h3 style="margin-bottom:12px">طلابك المسجّلون</h3>
      @if ($students->isEmpty())
        <p style="color:var(--ink-2)">لا طلاب بعد — انسخ رابطك وابدأ بمشاركته.</p>
      @else
        <div class="table-wrap">
          <table class="tbl">
            <thead><tr><th>الطالب</th><th>تاريخ التسجيل</th><th>الاشتراك</th></tr></thead>
            <tbody>
              @foreach ($students as $s)
                ...
                <tr>
                  ...
                  <td><span class="badge {{ $active ? 'ok' : 'muted' }}">{{ $active ? 'مشترك' : 'غير مشترك' }}</span></td>
                </tr>
              @endforeach
            </tbody>
          </table>
