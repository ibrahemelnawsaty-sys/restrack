  ...
  <div class="auth-wrap">
    <div class="glass auth-card rise in">
      ...
      <form method="POST" action="{{ route('register') }}">
        ...
        <div class="field">
          <label>هل تمّت دعوتك من قِبَل دكتور؟</label>
          <div style="display:flex;gap:18px;margin-top:4px">
            <label class="check"><input type="radio" name="invited" value="no" @checked(empty($referrer) && old('invited', 'no') === 'no')> لا</label>
            <label class="check"><input type="radio" name="invited" value="yes" @checked(!empty($referrer) || old('invited') === 'yes')> نعم</label>
          </div>
        </div>

        <div class="field" id="doctorPick" @unless(!empty($referrer) || old('invited') === 'yes') hidden @endunless>
          <label for="doctorSearch">اختر الدكتور الذي دعاك</label>
          <input id="doctorSearch" type="text" placeholder="ابحث باسم الدكتور…" autocomplete="off">
