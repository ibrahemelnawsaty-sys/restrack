...
@section('content')
  <section class="page">
    <div class="page-head rise in">
      ...
      <h1>{{ __('تفاصيل البرنامج قبل الدفع') }}</h1>
      <p>{{ __('راجِع ما ستحصل عليه، ثم أكمِل الاشتراك.') }}</p>
    </div>

    @if ($alreadySubscribed)
      <div class="alert rise in">{{ __('لديك اشتراكٌ فعّال بالفعل —') }} <a href="{{ route('dashboard') }}" style="color:var(--gold-ink);font-weight:700">{{ __('اذهب إلى لوحتك') }}</a>.</div>
    @endif
    ...
    <div class="split" style="grid-template-columns:1.5fr .9fr;align-items:start">
      ...
      <div class="glass tile rise in" style="position:sticky;top:90px">
        ...
        <div class="amt" style="display:flex;align-items:baseline;gap:.35rem;margin-top:14px">
          ...
          <span style="color:var(--ink-2)">{{ __('ر.س') }}</span>
