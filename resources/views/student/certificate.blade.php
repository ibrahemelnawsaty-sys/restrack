    ...
    <div class="glass cert rise in" style="max-width:680px">
      ...
      <div class="desc">
        ...
        @if ($certificate->type === 'final')
          {{ __('مسار') }} «<span style="direction:ltr;unicode-bidi:isolate">{{ $title }}</span>»
        @else
      ...
      <div style="display:flex;justify-content:center;margin-top:18px">
        ...
        @else
          <div style="width:132px;height:132px;border-radius:12px;border:1px dashed var(--g-border);display:grid;place-items:center;color:var(--ink-3);font-size:.72rem;text-align:center">{{ __('امسح للتحقّق') }}</div>
        @endif
      </div>

      <div class="foot">
        <span class="num">{{ $certificate->number }}</span>
        <a class="qr" href="{{ $verifyUrl }}" style="text-decoration:none;color:var(--gold-ink)"><svg class="ico" aria-hidden="true"><use href="#i-globe"/></svg>{{ __('تحقّق') }}</a>
