...
@section('title', __('محتوى الصفحات — الإدارة'))
...
@section('content')
  <section class="page">
    ...
    <div class="page-head rise in">
      <h1>{{ __('محتوى الصفحات') }}</h1>
      <p>{{ __('حرّر نصوص الموقع (العنوان، الوصف…) بالعربية والإنجليزية — تُحدَّث فوراً.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.content.update') }}" class="glass tile rise in">
      ...
      <button type="submit" class="btn btn-gold" style="margin-top:18px"><span class="sheen"></span>{{ __('حفظ المحتوى') }}</button>
    </form>
  </section>
@endsection
